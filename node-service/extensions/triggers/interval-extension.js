...
//
	...
	var addActivity = function(future, config, item, next, newFuture) {
		if(newFuture)
			future = newFuture;

		if(((item.intervalHours == 0) && (item.intervalMinutes == 0)) ||
			((item.activeHours == 0) && (item.activeMinutes == 0)) ||
			((item.intervalHours == item.activeHours) &&
			(item.intervalMinutes == item.activeMinutes)))
		{
			next(future);
		}
		else {
	...
	var delActivity = function(future, item, next, newFuture) {
		if(newFuture)
			future = newFuture;
